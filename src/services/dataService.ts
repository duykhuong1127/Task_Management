...
class DataService {
  ...
  // Admin Direct Authentication (Bypasses Gmail approval)
  public loginAsAdmin(): { success: boolean; user: User } {
    let admin = this.users.find((u) => u.role === 'ADMIN' && u.status === 'ACTIVE');
    if (!admin) {
  ...
  public checkApprovalStatus(identifier: string): { approved: boolean; user?: User; message: string } {
    const user = this.users.find(
      (u) => u.uid === identifier || u.normalizedEmail === identifier.trim().toLowerCase()
    );
    if (!user) {
  ...
  public loginWithGoogle(
    ...
    // Notify all Admins about this new registration request
    const adminUsers = this.users.filter((u) => u.role === 'ADMIN');
    adminUsers.forEach((admin) => {
      this.addNotification({
        userId: admin.uid,
        type: 'USER_REGISTERED',
        title: 'Yêu cầu đăng ký tài khoản mới',
        body: `Thành viên ${newUser.displayName} (${newUser.email}) vừa đăng ký bằng Gmail. Vui lòng xét duyệt tài khoản.`,
        deduplicationKey: `REG_${newUser.uid}`,
      });
    });

    // Send direct notification for the user
    this.addNotification({
      userId: newUser.uid,
      type: 'ACCOUNT_STATUS',
      title: 'Đăng ký tài khoản thành công',
      body: 'Vui lòng chờ admin xét duyệt tài khoản.',
      deduplicationKey: `PENDING_${newUser.uid}`,
    });
