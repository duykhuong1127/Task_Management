...
const StatusBadge: React.FC<{ task: Task }> = ({ task }) => {
  ...
  return (
    <span className={`text-[9px] uppercase tracking-wider px-2 py-0.5 rounded border ${classes}`}>
      {statusLabel[status]}
    </span>
  );
};

const TaskCard: React.FC<TaskCardProps> = ({ task, onOpen, draggable = false }) => {
    ...
    >
      ...
      <div className="mt-3 h-1.5 bg-[#222] rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${Math.max(0, Math.min(100, task.progressSummary))}%` }}
        />
      </div>
...
export const MultiViewTasks: React.FC<MultiViewTasksProps> = ({
  ...
  const counts = useMemo(
    () => ({
      ALL: scoped.length,
      ASSIGNED_TO_ME: scoped.filter((task) => task.assigneeIds.includes(currentUser.uid)).length,
      ASSIGNED_BY_ME: scoped.filter(
        (task) => task.assignerId === currentUser.uid || task.createdBy === currentUser.uid
      ).length,
      OVERDUE: scoped.filter((task) => task.status !== 'COMPLETED' && isOverdue(task.deadline)).length,
      DUE_SOON: scoped.filter((task) => task.status !== 'COMPLETED' && isWithin72Hours(task.deadline)).length,
  ...
  const filtered = useMemo(() => {
    ...
    const rows = scoped.filter((task) => {
      if (text && !`${task.taskId} ${task.title} ${task.description}`.toLowerCase().includes(text)) return false;
      if (filter === 'ASSIGNED_TO_ME') return task.assigneeIds.includes(currentUser.uid);
      if (filter === 'ASSIGNED_BY_ME') {
        return task.assignerId === currentUser.uid || task.createdBy === currentUser.uid;
      }
  ...
  const timelineMonthEnd = new Date(
    ...
    <div className="space-y-5 animate-in fade-in duration-300">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 pb-4 border-b border-[#222]">
        <div>
          <div className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF37] font-semibold">
            Không gian công việc đa góc nhìn
          </div>
          <h1 className="text-2xl font-serif italic text-white mt-1">Công Việc</h1>
          <p className="text-xs text-[#777] mt-1">
            Một nguồn dữ liệu, bốn cách quan sát: danh sách, Kanban, lịch và timeline.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-md text-[11px] flex items-center gap-1.5 transition-colors ${
                    active ? 'bg-[#D4AF37] text-black font-semibold' : 'text-[#888] hover:text-white'
                  }`}
                >
          ...
          <button
            ...
            className="px-3.5 py-2 rounded bg-[#D4AF37] text-black text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"
          >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[11px] whitespace-nowrap border transition-all ${
                filter === id
                  ...
                  : 'bg-[#111] border-[#252525] text-[#888] hover:text-white'
              }`}
            >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 text-[#777]">
            ...
            <select
              ...
              className="bg-[#111] border border-[#252525] rounded px-2 py-2 text-xs text-[#bbb] focus:outline-none"
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {filtered.map((task) => (
            <TaskCard key={task.taskId} task={task} onOpen={() => onSelectTask(task)} />
          ))}
        </div>
      ) : viewMode === 'BOARD' ? (
            ...
            {boardColumns.map((column) => {
              ...
              return (
                <section
                  ...
                  className={`rounded-xl border p-2.5 min-h-[520px] transition-colors ${
                    highlighted ? 'border-[#D4AF37] bg-[#D4AF37]/5' : 'border-[#242424] bg-[#0A0A0A]'
                  }`}
                >
                  <div className="flex items-center justify-between px-1 pb-2.5">
                    <h2 className="text-xs uppercase tracking-wider text-[#bbb] font-semibold">{column.title}</h2>
                    <span className="text-[10px] bg-[#1A1A1A] text-[#777] rounded-full px-2 py-0.5">
                      {columnTasks.length}
                    </span>
                  </div>
                  {!column.editable && column.status === 'COMPLETED' && (
                    <div className="text-[9px] text-[#666] px-1 pb-2">
                      Hoàn thành chỉ qua quy trình nghiệm thu.
                    </div>
                  )}
                  <div className="space-y-2.5">
        ...
        <div className="rounded-xl border border-[#252525] bg-[#0A0A0A] overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#252525]">
            <button onClick={previousMonth} className="p-1.5 rounded hover:bg-[#1A1A1A] text-[#aaa]">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div className="text-sm font-semibold text-white">
              Tháng {calendarCursor.getMonth() + 1}/{calendarCursor.getFullYear()}
            </div>
            <button onClick={nextMonth} className="p-1.5 rounded hover:bg-[#1A1A1A] text-[#aaa]">
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
          <div className="grid grid-cols-7 border-b border-[#252525] bg-[#0E0E0E]">
            {['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map((day) => (
              <div key={day} className="p-2 text-center text-[10px] uppercase tracking-wider text-[#666]">
                {day}
              </div>
            ))}
          </div>
            ...
            {calendarDays.map((date) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[120px] border-r border-b border-[#1E1E1E] p-1.5 ${
                    inMonth ? 'bg-[#0A0A0A]' : 'bg-[#080808] opacity-50'
                  }`}
                >
                  <div
                    className={`text-[10px] w-6 h-6 grid place-items-center rounded-full mb-1 ${
                      today ? 'bg-[#D4AF37] text-black font-bold' : 'text-[#777]'
                    }`}
                  >
                    {date.getDate()}
                  </div>
                  <div className="space-y-1">
                    {rows.slice(0, 3).map((task) => (
                      <button
                        ...
                        className={`w-full text-left rounded px-1.5 py-1 text-[9px] truncate border ${
                          effectiveStatus(task) === 'OVERDUE'
                            ? 'border-rose-900/50 bg-rose-950/30 text-rose-300'
                            : effectiveStatus(task) === 'COMPLETED'
                              ? 'border-emerald-900/40 bg-emerald-950/20 text-emerald-300'
                              : 'border-[#2D2D2D] bg-[#151515] text-[#bbb]'
                        }`}
                        title={task.title}
                      >
                        {task.title}
                    ...
                    {rows.length > 3 && (
                      <div className="text-[9px] text-[#666] px-1">+{rows.length - 3} việc khác</div>
                    )}
                  </div>
                </div>
              );
        ...
        <div className="rounded-xl border border-[#252525] bg-[#0A0A0A] overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#252525]">
            ...
            <div>
              <div className="text-sm font-semibold text-white text-center">
                Timeline tháng {calendarCursor.getMonth() + 1}/{calendarCursor.getFullYear()}
              </div>
          ...
          <div className="divide-y divide-[#1E1E1E]">
            {filtered
              .filter(
                (task) =>
                  new Date(task.deadline) >= timelineMonthStart && new Date(task.createdAt) <= timelineMonthEnd
              )
              .map((task) => {
                ...
                return (
                  <button
                    key={task.taskId}
                    onClick={() => onSelectTask(task)}
                    className="w-full grid grid-cols-[220px_1fr] gap-4 p-3 hover:bg-[#101010] text-left"
                  >
                    <div className="min-w-0">
                      <div className="text-xs text-white truncate">{task.title}</div>
                      <div className="text-[9px] text-[#666] mt-1">
                        {task.taskId} · {task.progressSummary}%
                      </div>
                    </div>
          ...
          {filtered.every(
            (task) => new Date(task.deadline) < timelineMonthStart || new Date(task.createdAt) > timelineMonthEnd
          ) && (
            <div className="p-10 text-center text-xs text-[#666] flex items-center justify-center gap-2">
              <AlertTriangle className="w-4 h-4" /> Không có công việc giao với tháng này.
            </div>
          )}
        </div>
      )}
